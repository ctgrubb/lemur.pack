...
For example, lets look at $k = 100000$ bridges with $5$ intermediate points, and focus on $B(v)$ with $v = 2, 4$. We know the expected value will always be zero, so we focus on the covariance matrix. Empirically, we get
...
solve(sigma[c(2, 4), c(2, 4)])
```

which can be turned into

\[
  x^T \Omega x = 5 {x_1}^2 - 5 x_1 x_2 + 5 {x_2}^2
\]
