...
And we can compare the time it takes for each method.

```{r denseval-times, echo = FALSE, out.width = "100%", fig.cap = "Time comparison for methods"}
par(mar = c(2.5, 2.5, 1, 1))

boxplot(microbenchmark(denseval.slow(b, sigma), denseval.fast(b), times = 1000, setup = replicate(3, c(denseval.fast(b), denseval.slow(b, sigma)))), log = FALSE)
```
